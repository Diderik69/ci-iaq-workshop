pipeline {
    ...
    stages {
        ...
        stage('Run Testinfra Tests') {
            steps {
              sh "py.test --connection=ansible --ansible-inventory inventory/test.hcloud.yml --hosts='ansible://DiRei-ansible-test-instance' --force-ansible -v test/*.py" // hostnamen anpassen
            }
        }
    }
    post {
